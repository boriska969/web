...
// ----------------------------------------------------------------------------
// Checking if the string "str" contains a number (integer or decimal)
...
// ----------------------------------------------------------------------------
// Checking if the string "str" contains a single digit
...
// ----------------------------------------------------------------------------
// Checking if the string "str" contains a single operator
...
// Разбивает исходную строку на токены, объединяя последовательности цифр в числа
...
// Преобразует инфиксную запись в ОПЗ (RPN) алгоритмом сортировочной станции
...
function evaluate(str) {
    ...
    for (const t of tokens) {
        ...
        else throw new Error('invalid token'); // неизвестный токен
        stack.push(res);
    }
    if (stack.length !== 1) throw new Error('invalid rpn'); // некорректное выражение
    return stack[0];
}
